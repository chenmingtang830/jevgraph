"""Build the offline interactive report from the committed aggregate evidence.
...
def report_data(evidence: dict[str, object]) -> dict[str, object]:
...
def build(*, evidence_path: Path, template_path: Path, output_path: Path) -> None:
    evidence = json.loads(evidence_path.read_text(encoding="utf-8"))
    payload = json.dumps(report_data(evidence), ensure_ascii=False, separators=(",", ":"))
...
def main() -> None:
    ...
    build(evidence_path=args.evidence, template_path=args.template, output_path=args.out)
